...
RSpec.describe "Account Theme Preferences", type: :request do
  describe "unauthenticated access" do
    it "redirects PATCH /account/theme_preference to sign in" do
      patch settings_theme_preference_path
  ...
  context "authenticated" do
    ...
    describe "PATCH /account/theme_preference" do
      it "updates the theme" do
        patch settings_theme_preference_path, params: { user_preferences: { theme: "dark" } }
      ...
      it "rejects an unknown theme with an alert, to the same destination" do
        ...
        expect(flash[:alert]).to eq(I18n.t("settings.theme_preferences.update.invalid_theme"))
        expect(user.preferences.reload.theme).not_to eq("chartreuse")
      end
    ...
    describe "PATCH with invalid theme" do
      it "redirects with alert for invalid theme" do
        patch settings_theme_preference_path, params: { user_preferences: { theme: "neon" } }
        expect(response).to redirect_to(edit_settings_profile_path)
      end
    end
    ...
    describe "Pundit authorization wiring" do
      it "raises NotAuthorizedError → redirects when the policy denies update" do
        allow(Settings::ThemePreferencesPolicy).to receive(:new)
          .and_return(instance_double(Settings::ThemePreferencesPolicy, update?: false))
