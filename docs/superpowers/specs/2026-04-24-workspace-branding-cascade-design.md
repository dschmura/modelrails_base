...
- **Hue-dependent AAA contrast guarantee.** The `oklch(0.40 0.15 <hue>)` formula produces AAA-level (7:1) contrast on white for most hues, but OKLCH perceived lightness varies with hue at a fixed `L`. Yellow-green hues (~80–140) land perceptually lighter and can fall below AAA, occasionally below AA. The model validation allows any hue 0–360, so a workspace owner who picks a hue in that band gets buttons with non-guaranteed contrast. This is a known gap flagged in the user-facing docs ([app/docs/ui-patterns.md](app/docs/ui-patterns.md) → Workspace Branding subsection) and will be addressed alongside the color-strategy unification — likely via a contrast-aware lightness computation (higher `L` for yellow-green hues) or a hue-range restriction in the picker.
