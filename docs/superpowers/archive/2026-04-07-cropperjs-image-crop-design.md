...
- **EXIF orientation:** Photos from mobile devices often include EXIF rotation metadata (portrait stored as landscape + rotation flag). Cropper.js v1 supports EXIF orientation correction via its `checkOrientation` option (enabled by default). This must be explicitly tested — upload a portrait phone photo and verify the crop UI shows it upright.
