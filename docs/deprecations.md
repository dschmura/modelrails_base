...
### Current call-sites (as of 2026-06-17)

The `personal?` predicate is read directly in three presentation-layer locations:

- **`_settings_sidebar_switcher.html.erb`** — avatar shape: `personal?` → `rounded-full`; org → `rounded-md overflow-hidden`.
...
Under `:none`, every `personal?` branch in the presentation layer (the sidebar switcher's avatar shape, `settings_navigation_helper#settings_context_kind`, `workspace_helper#workspace_icon_for`) is dead — yet it's read directly by views/helpers. That's a *presentation leak*: how a workspace is shown/treated is decided by scattering `Workspace#personal?` reads across the UI rather than asking one posture-aware seam.
