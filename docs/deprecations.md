...
### Current call-sites (as of 2026-06-19)

The `personal?` predicate is read directly in one presentation-layer location:

- **`workspace_helper#workspace_icon_for`** — falls back to the workspace owner's avatar only when `workspace.personal?`.

Note: `settings_navigation_helper#settings_context_kind` was removed in Phase 2
(`feat/workspace-settings-ia-phase2`). The settings sidebar now selects its partial
via `SettingsContext` declared on each controller — no `personal?` read at render
time. `_settings_sidebar_switcher.html.erb` also reads `personal?` for avatar shape,
but that partial is no longer rendered from the settings sidebar (removed in Phase
2c-1 Task 2 — the header switcher supersedes it). It remains in use from
`_workspace_sidebar_items.html.erb`.
...
Under `:none`, every remaining `personal?` branch in the presentation layer (`workspace_helper#workspace_icon_for`) is dead — yet it's read directly by helpers. That's a *presentation leak*: how a workspace is shown/treated is decided by scattering `Workspace#personal?` reads across the UI rather than asking one posture-aware seam.
