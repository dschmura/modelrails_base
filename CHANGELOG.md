...
- **The `:user` factory now creates a verified email authentication by default.** Every production signup makes one — magic-link registration stamps it verified, password-set creates it pending — so a user without one was a state the app could not reach, and `User#can_invite?` answered `false` for the universal case. The traits invert accordingly: `:with_verified_email_auth` is **gone** (it is the default now), `:with_email_auth` is **renamed `:unverified_email`**, and the old authless state is `:no_authentications`. `spec/` is not a fork seam, so this arrives on sync: a fork's specs that hand-build an email authentication must start from `:no_authentications` (the factory's row collides — `provider` is unique per user), and any spec relying on a bare `create(:user)` being unable to invite must name `:unverified_email` or `:no_authentications` (#850).
