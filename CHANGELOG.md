...
- `bin/fork --template URL` names the template when it is a GitHub fork's missing `upstream` or a synced mirror under another name: the URL is recorded in `.fork.yml` for `bin/setup`, an `upstream` equal to it counts as configured, and an `origin` equal to it gets the template's remote surgery. Equality only, `.git`-insensitive; the `/modelrails_base` name anchor is unchanged.
