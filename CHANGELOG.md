...
- **The audit snapshot is derived, not accepted — and a deleted actor no longer shortens the busiest-actors strip.** A review panel on the #1122 snapshot found four things worth fixing, two of them real defects rather than notes. The snapshot callback guarded on `actor_name.nil?`, which defended a caller that does not exist and in exchange let one write `actor_name: "Someone Else"` next to a contradicting `actor_id` into a row that is **immutable afterwards** — the assignment is now unconditional, and on `before_create` rather than `before_validation`, so it no longer mutates on a bare `valid?` that never saves. The second is a regression the snapshot itself opened: the operations ledger's "most active" strip claims its slots by `actor_id` in SQL and resolves names afterwards, so a departed heavy actor took a slot and then vanished, leaving the strip rendering four while calling itself the five busiest. It now over-fetches and re-limits. Also: the migration priced every design decision except its own runtime (`remove_foreign_key` on SQLite is a full table rebuild under the writer lock, on the largest table in a mature fork), the docs implied `user.destroy` now works when it was one of six blocked columns (#1248), a comment claimed renames never travel backwards when pre-snapshot rows still follow them, and `encrypts :actor_name` was **provably unpinned** — deleting it left all eight new examples green. It is now in the at-rest inventory with a non-determinism check. (#1122)

- **An audit row now outlives the people in it.** `activity_logs.actor_id` carried a real foreign key to `users` with no cleanup path, so any user who had ever acted — which is any user who has done anything — could not be destroyed. Latent only because `user.destroy` has no call site yet; a fork adding account deletion hits it on a table it has no reason to think about. Both obvious fixes are wrong: `dependent: :destroy` deletes history to delete a person, and `dependent: :nullify` rewrites immutable rows to hide who acted — the immutability guard already fences that spelling by name, for this issue. So the row **snapshots the actor's name** at write time into an encrypted `actor_name` column and the foreign key is gone. The historical fact stops depending on the user row surviving, and stops following a later rename, which is what an audit table wanted anyway. `trackable_id` has never had a foreign key, so both columns now fail the same way rather than in opposite directions. Deliberately **not** backfilled — an `UPDATE` across the table to add a field is the rewrite `ActivityLog#readonly?` exists to prevent; pre-existing rows fall back to the association and read as a former member once it no longer resolves, never as `System`. One knock-on the specs caught: with `display_subject` reading the snapshot, the feed loader's blanket `includes(:actor)` became an unused eager load on three feeds, so actors are now preloaded only for pre-snapshot rows, and the operations ledger — the one row that reads the actor's live email, for its "only this person" pivot — preloads them itself. (#1122)

- **The client area's five flashes are asserted, not just their redirects.** The fourth #526 slice, 41 entries to 36, and the one where a redirect proves least: a client who asks for a draft resource, an unshared one, a project they hold no access to, or any resource in a project whose owner switched Clientside off is sent to one of two pages, and **the message is the only thing that says which refusal happened**. Two of those keys deliberately share a message — a client is told a resource is out of reach, never why — and that is now pinned rather than assumed, so narrowing it to "this one is still a draft" has to be a deliberate edit. The team-side pair covers the Clientside-off guard on the invite form and the invalid-email re-render, which sits beside an already-invited re-render that returns the same 422. (#526)
...
- **Theme resolution has one home.** It was written twice — in the inline pre-paint script and in `theme_controller.js#applyTheme` — and that duplication is nastier than most, because the two can only disagree during the window *before* Stimulus boots: the symptom is a paint flash that no spec watches and no user can report precisely. It cannot simply move into the controller, since importmap modules are deferred and a module-side answer arrives after first paint, which is the whole reason the inline script exists. So the script now **defines** `window.ModelRails.themeIsDark` (and the shared `matchMedia` list the controller also listens on), and the controller calls it. That makes the controller depend on the script having run, so a code-smell spec pins the thing that keeps it true: every layout instantiating the `theme` controller also renders the script — with a positive control, since a scan that matches nothing asserts nothing. Worth noting the preview layout is *not* affected: it drives theme from Lookbook's toolbar server-side and never instantiates the controller. The first-paint behaviour itself is still proven by #1229's `readyState` recorder. (Addresses #624)

### Fixed

- **The test browser reaches no third-party host.** Two `embed` previews still fetched YouTube and Google Maps after #1207 fixed the other six, and they survived the network going down only because their assertion used `visible: :all` — which matches an element that has not laid out. That was luck, not design: tightening that matcher the way every sibling component spec does would have reintroduced the failure. The `data:` swap that fixed the others does not transfer here, because `EmbedComponent` turns a caller's `url:` into the iframe `src`, so a watch URL *is* the subject being demonstrated. **`Network.setBlockedURLs` is not enough either** — measured, it stops subresources but not an iframe's own document, so `youtube.com/embed/…` still returned 200 with blocking "on", and the specs passed either way. The fetch is stopped at the browser instead: `host-resolver-rules` resolves those hosts to nothing for every system spec, taking off-host traffic on that preview from **12 requests to one that fails**. Checked before going global — the app's CSP is `font_src :self, :data`, so blocking `fonts.gstatic.com` cannot shift its text metrics, and the whole suite passes with the hosts gone. The two specs now assert the **derived embed URL**, which is the transformation the preview exists to show and was never checked at all. The preview itself is untouched, so a real embed still renders for anyone browsing Lookbook in development. (#1233)

### Fixed

- **The operations member-order spec measures the member list, not the page around it.** It asserted ordering with `body.index(last_name)` over the whole document, and three things could answer: the **operator's own Faker name** in the user-menu `aria-label` near the top of the page, the activity feed further down listing the same people in *recency* order, and the `Owner` role badge, which is not a person at all. It failed whenever Faker handed the operator a name containing Adams, Baker, Owner or Young — deterministic per seed rather than flaky, and it named the sort rather than the chrome, so whoever hit it bisected their own change first. It now reads the list's own rows in DOM order and compares rendered text. The behaviour under test — ordering by decrypted names, which cannot be done in SQL — is unchanged, and removing the sort still fails it. (#1231)
- **A tinted chip has a visible edge again.** All eight signal `-border` values were under the 3:1 non-text floor (WCAG 1.4.11) against this app's own grounds — **1.34–2.89:1**, every tone, both themes. That matters because the border is the *only* boundary an alert, banner, toast, badge or error summary has: a chip's fill measures ~1.06:1 against the page and **cannot** be darkened, since its own text has to hold 7:1 against it (the darkest tint that still does reaches 1.09–1.29:1). The values adopt `modelrails_ui` v0.23.0's, which land at **3.10–3.12:1** — hue and chroma untouched, lightness repositioned, each solved against the *worst* ground in its theme (`surface` in light, `surface-raised` in dark) rather than the convenient one, because solving against white and the dark page produces values that pass on the page and fail inside a card. The gem shipped these in its own stylesheet; this app keeps its own copy of the tokens, and **nothing on this side was watching it** — the gem's contrast test reads the gem's stylesheet, so the app's copy simply stayed wrong. A new code-smell spec now computes the ratios here. The colour math it needs is the same arithmetic the toast gate added, so it moves to `spec/support/contrast_math.rb` rather than existing twice: two copies of a colour-space conversion is exactly the drift these guards exist to catch. (modelrails_ui#257)

### Fixed

- **Toast icons are visible again.** In dark mode a success toast's check measured **1.32:1** against its pill (info 1.40:1), where the non-text floor is 3:1 — reported from the browser as "almost invisible". The pill is an **inverted surface**: `--color-surface-toast` is neutral-900 in the light theme and neutral-100 in the dark one, so a token tuned for the page's own ground is wrong on it by construction. The text already had `--color-text-on-toast` for exactly that; the icon rode the page-ground `--color-*-icon` and painted a green-400 onto a near-white pill. Inverting the *icon* token is not enough (1.65–1.83:1 — those values are mid-tone); the pill now takes the opposite theme's **AAA text** value, which reuses something already proven at 7:1 and clears the floor with margin at **6.75–7.69:1** in both themes. Measuring the cards then turned up a second, older failure with nothing to do with inversion: a **`warning` icon on `warning-surface` at 2.06:1 in light mode**, while the message beside it sat at 8.62:1 on the same ground — so card icons now take the tone's text token, as the gem's own alert already does via `text-current`. A new code-smell spec computes every ratio from the shipped token values, compositing the pill's 90% alpha over the page because measuring the raw token flatters it, and pins both the cross-theme mirror and the config wiring — a correct token nobody references fixes nothing. **axe measures text contrast only**, so nothing else could have caught any of this. (#1236)

### Fixed

- **Every form control's invalid state survives forced-colors mode.** The `modelrails_ui` pin moves to `v0.23.0` and the vendored copies adopt it. The invalid state was carried by `aria-invalid:ring-*` — a box-shadow, which forced-colors/High-Contrast does not paint — so eleven controls degraded to a pale `danger-border` edge and **`range` and `switch` showed no invalid state there at all**. It is a border *width* change now (`border-2 border-danger`), and the width is the half that matters: the system repaints every border in forced-colors, so a colour swap alone leaves valid and invalid identical. `range` puts the edge on its track pseudo-element (its own box is the transparent 44px interaction area) and `switch` thickens the track's existing border. The banner's dismiss control also stops fading on hover — an opacity fade reads as *disabled*, which is the opposite of what a dismiss control does — and tints per variant instead. Separately, 32 sites adopt `min-h-input` in place of raw `min-h-11` / `min-h-[var(--form-input-height)]`: nothing renders differently today, but `--form-input-height` is the documented retune knob, so a fork setting it to 48px would previously have moved some controls and stranded others. **The `min-h-input` gate widens accordingly** — its own comment said to do this "once the gem-side sweep lands" — now covering `app/components/ui/**` and `app/form_builders/**`, which is how a missed site in the vendored form builder surfaced. One exemption, guarded against rotting into a no-op: `mega_menu`'s item is a navigation link at the 2.5.5 target floor, not a form control, so the form-control knob should leave it alone. **One upstream defect is deliberately not adopted**: v0.23.0's switch carries a find/replace remnant (`"peer-peer-"`) that swallows the adjacent class and breaks the disabled fade, filed as `modelrails_ui#263`; this app lands the corrected line, so the two differ until that ships. (modelrails_ui #252/#256/#258)

### Fixed

- **A docs query typed before the search index arrives is no longer dropped.** The `markdowndocs` pin moves to `0.11.4`, which carries two search fixes that had sat unreleased on that gem's `main` since 2026-09-08. The one that mattered here: `performSearch` returned through `showAll()` while `miniSearch` was still null and nothing re-read the input once the fetch resolved, so on a slow connection the docs filter looked dead to anyone who started typing straight away. The gem shipped that fix **without a test on purpose** — it has no JavaScript harness — so the proving spec lives here, where Cuprite already drives the gem's own controller. The race is made deterministic rather than waited on: the index response is held at the browser with `cdp_intercept`, the query is typed while it is in flight, the 50ms debounce is allowed to fire against a null index, and only then is the response released. **The first draft of that spec passed against the unfixed gem too** — it released the response immediately, so `performSearch` ran with the index already in place and the race never happened. It now fails against `0.11.3` on the right assertion and passes against `0.11.4`, which is the only thing that makes it evidence. The `path_slug` fix in the same release needed no action: this app overrides `docs/index.html.erb`, which a gem upgrade does not reach, and that override already carried the fix locally. The bump is `--conservative` — a plain `bundle update` also dragged `commonmarker` 2.9.0 → 2.10.0, a markdown-renderer upgrade that does not belong in a pin bump. (#1074, markdowndocs#42)

### Fixed

- **The suite stops reaching the public internet to render six component previews.** An avatar, a QR code and two iframes fetched from `i.pravatar.cc`, `api.qrserver.com`, `www.openstreetmap.org` and `player.vimeo.com`, and the system specs auditing those previews waited on the fetch — so a machine with no network got three failures naming the avatar and iframe components, which **refused a push twice in one evening** through Lefthook's pre-push gate. They are `data:` URIs now (an inline SVG for each image, a one-line document for each frame), matching the upstream fix in `modelrails_ui` v0.23.0. Only the `src:` values changed: four of these previews carry `#if-scope`/`#av-scope` wrappers that the app's own axe scoping depends on and the gem's copies do not have, so a blanket re-vendor would have deleted them. Proven with every `http(s)` request blocked at the browser — all four previews render, the frame reads "Embedded map", in 1.2s. A code-smell spec now fails on any preview passing an off-host absolute URL as `src:`; it checks `src:` only, because a preview may legitimately *display* a URL (`copy`'s invitation link, `device_mockup`'s address bar) without fetching it. Two embed previews still fetch YouTube and Google Maps and are filed separately (#1233) — there the URL is the subject being demonstrated, not a fixture, so the same swap would not work. (#1207, modelrails_ui#253)
### Changed

- **The operations users page is a real index.** It was an exact-email lookup rendering a list of at most one row — a search-results shape for what was really a `find`, and the duplicated row had already drifted from the user's own page (two badges against three, missing `locked_out` against its own spec's description). It is now a paginated list of everyone with an account, newest first, with a filter over it. **The order is `created_at` and cannot be a name**: names are encrypted non-deterministically, so an SQL sort reads ciphertext and a keyed digest is not order-preserving — the only SQL-orderable name key would be a plaintext column, which is a security decision rather than an oversight. The filter keeps two branches for the same reason: an exact match on the deterministically-encrypted address, and a bounded decrypt-and-match pass over names. **Both caps are stated on the page** rather than applied quietly — a list that silently drops matches is worse than one that admits it, because an operator cannot otherwise tell "no more" from "not shown". The name-matching rule moves to `User::Search`, which the activity ledger's search box now shares, so the decrypt-and-match doctrine and its CPU budget have exactly one home; and the state badges move to one partial the index row and the show page both render, so the drift that prompted this cannot recur. Reach comes from a new `User#operated_users` relation rather than `User.all`, so scoped operators narrow it in one line. (#1135)

### Added

- **Blocked senders** is a page in account settings, and unblocking is self-serve. Blocks are created from the *unauthenticated* decline page — including by people who have no account — and removal had only an operator door: a two-statement console recipe in the troubleshooting doc. The person who mis-clicked had no undo they could reach. The list names the blocks made against the address you sign in with, and **Allow again** lifts one. Two things this deliberately does not do, both because blocks are **email-keyed and account-independent**: a block against an address you no longer sign in with is not listed (it never followed you there), and a block made before you had an account *is* (it was keyed on the address all along). `InvitationBlock.unblock!` replaces the console recipe in one transaction — destroying the block alone leaves the stamped invitation invisible to `bulk_invite!`'s duplicate check, so every later invite to that address mints a duplicate rather than sending, which is exactly the half a two-statement recipe loses. A foreign block id answers as not-found rather than forbidden: a refusal would confirm the id names a real block, telling the requester that an address they do not own has blocked someone. (#812)

- Unsaved document edits survive a closed tab. The resource forms opt into draft recovery, which the feature had declared rich text out of scope for — on the belief that a Lexxy body lives in a hidden input, which `recover()` refuses to write back. Measured against the real editor, that was never true: `<lexxy-editor>` is a **form-associated custom element** that carries the `name`, participates in `FormData`, and re-renders from its `value` setter, so the generic restore path already handled it. The only thing missing was a save trigger, because the editor emits `lexxy:change` and neither `input` nor `change`. The hidden `resource[type]` field is marked `data-form-draft-ignore`: a hidden field with no visible sibling downgrades every restore to the partial announcement, which would tell a screen reader user content was left behind when the editor had in fact been refilled. The two announcements that named rich text specifically now describe the condition they actually test. The restored count reads one high, because the editor's toolbar carries a named `<select>` that the serializer treats as one of the form's fields — that is `modelrails_ui#262`, and it is documented rather than worked around. (#479)

### Fixed

- The inline theme script stops reading a cookie it can never see. Its fallback fired only when `data-theme-theme-value` said `system`, and that attribute is `current_user_theme`, which reads the same cookie first — so the branch was unreachable, and it was the one place where the pre-paint script and `theme_controller.js` disagreed about where a theme comes from. The example covering this asserted `html.dark` after a full load, which `theme_controller` also satisfies, late enough to flash the wrong theme first — exactly what the inline script exists to prevent. It now records `document.readyState` at the moment `dark` first appears, from a recorder injected ahead of every page script, and requires `loading`. Mutating the inline class-add out leaves the old assertion green and fails the new one. (Addresses #624)
...
- `date_picker` and `sidebar` catch up to the gem after sitting behind it: the date picker gains the typed entry path it has lacked since v0.17.0, and the sidebar finally emits the `id` it was already computing. Neither has a call site in this app, which is why both drifted unnoticed — and why closing the gap now costs nothing. `navigation_menu` was **hand-merged instead of regenerated**: its `PANEL_LINK` carries a 44px AAA target floor the gem does not ship, and `--force` reverts it silently, since this app never audits that component in isolation. The constant now says so and points at the upstream issue (modelrails_ui #246). `Metrics/ClassLength` now skips the two vendored trees: the gem's addition pushed `form_builder` one line over the app's limit, and a size complaint about generator-owned code is unactionable — shortening it is fork divergence, and the next `modelrails_ui:add` reverts it. Everything the app writes is still measured.
...
- Every hand-rolled empty-state block is now `UI::EmptyState`: the shared `shared/_empty_state` partial behind 13 call sites, the passkeys card, and both markdowndocs states. Two defects retire with the copies — the passkeys card painted `bg-surface` on a bordered container, which is the colour of the page beneath it, and the markdowndocs states emitted an `<h3>` for transient content, injecting a heading at an arbitrary level into the page outline. Titles are a `<p>` now. The one-line "no activity" notice in `settings/sessions` is deliberately left alone: it is a row-level notice, not an empty-state block. Slotted icons carry `mb-3` at the call site until the component spaces its own icon slot (modelrails_ui #241). (modelrails_ui #135)
...
- **Fork invariant — an instance has operators, and `/operations` is theirs.** `Operatorship` (a `Discardable` row per operator in a new `operatorships` table, partial unique index on kept rows) marks who administers the instance from above the workspaces. `Operations::` controllers under `/operations` — workspaces, users, activity, operators; 16 routes — answer 404 to anyone else and force reauthentication on every request. Under the `:shared` preset the seed grants the bootstrap owner an operatorship; elsewhere `rails operators:grant[email]` mints the first one. Landing with it: user suspension is a reversible hold (`users.suspended_at`; `User#suspend!(by:)` / `#unsuspend!(by:)` — sign-in refused at the one session funnel, sessions ended, memberships untouched) and replaces the evicting `users:suspend`; `User#unlock!(by:)` is the audited administrative lockout clear; `ActivityLog.record_security_event!` takes `actor:` and `visibility:` and is the one writer for every `SECURITY_ACTIONS` member (five new: `user.suspended`, `user.unsuspended`, `user.unlocked`, `operatorship.granted`, `operatorship.revoked`); `Suspendable#suspend!` / `#unsuspend!` are guarded inside their transaction and return an outcome; `Workspace#owner` ignores discarded memberships; `Workspace.create_for_owner_email` is the operator-create verb; `Settings::ReauthenticationsController` and `Settings::ReauthenticationCodesController` skip the onboarding requirement; `TENANCY_WORKSPACE_CREATION=disabled` binds operators too. (#1127; closes #1120)
  ...
  |---|---|
  | `rails users:suspend` no longer removes the user from their workspaces | It is a sign-in hold now (`users:unsuspend` lifts it); remove memberships from the members page or with `Membership#deactivate!` |
  | `NoMethodError: suspend_access!` | `user.suspend!(by:)`; nothing is discarded any more |
  | `ArgumentError: missing keyword :by` from `User#unlock!` | Pass the acting user — `by: Current.user`, or `by: nil` from a script |
  | `if workspace.suspend!` treated as a boolean | Case on `:suspended` / `:already_suspended` (and `:unsuspended` / `:not_suspended`) |
  | A direct `ActivityLog.create!` for a `SECURITY_ACTIONS` action fails `security_events_route_through_writer_spec` | Route it through `record_security_event!` (`actor:` / `visibility:` override the self-event shape), or register a best-effort writer in `spec/support/security_event_writers.rb` |

- Fork invariant — the workspace switcher is one partial, and it left the header. `shared/_workspace_switcher.html.erb` (its trigger `shared/_workspace_switcher_trigger.html.erb`) heads the sidebar and, on phones, renders again in the content column above the section tabs with `id_suffix: "-mobile"`; the header no longer renders it and the hamburger no longer lists workspaces. Gone with it: `shared/_workspace_identity_bar`, `shared/_workspace_name_link`, `shared/_workspace_context_banner`, and the `workspaces.context_banner.*`, `workspaces.identity_bar.*` and `workspaces.overview.customize.*` locale keys. A fork that overrode any of those re-applies the change to the one partial; the rename broadcast (`workspaces/update.turbo_stream.erb`) replaces the whole trigger, so markup a fork adds inside it must stay free of focusable elements. (#1078, #1089)
- Fork invariant — `.kamal/secrets` holds references, never literals. `spec/code_smells/kamal_secrets_hold_no_literals_spec.rb` fails the suite on any tracked `.kamal/secrets*` value that does not start with `$` — `$VAR` from the environment or `$(…)` from a password manager — and the deployment guide now says where the values live instead of instructing them into the tracked file. Failures name keys and line numbers, never values. (#1081)
...
- A fork names its nouns once. `config/vocabulary.local.yml` (fork-owned, `merge=ours`) overrides `workspace` and `project` (singular and plural); template copy says `%{workspace}`/`%{project}` and a boot-time backend hook fills the tokens, so every upstream string — present and future — renders in the fork's words. Two code-smell specs keep upstream honest (no bare noun in locale values; every interpolation token supplied). Eleven strings were reworded so no indefinite article precedes a noun. Models, routes, URLs and the user docs are unchanged by design (#1109). During the rewrite `%{project}` turned out to already carry a project's name in three strings (the second-noun twin of #1108); it is `%{project_name}` now, with a spec that counts the noun once and the name once. A third code-smell spec makes spec copy assertions go through `I18n.t`, with an allowlist for preview fixture text, so a renamed fork's suite stays green.
- `bin/fork` works from a fork of a fork. On a GitHub fork with no `upstream` yet, and nothing else naming the template, it asks `gh` for `origin`'s parent and records that in `.fork.yml` for `bin/setup` (not on the clone path, where `--origin` marks `origin` as the template itself). `--template URL` names the template explicitly and always wins: an `upstream` equal to it counts as configured, an `origin` equal to it gets the template's remote surgery. Equality only, `.git`-insensitive; the `/modelrails_base` name anchor is unchanged.
...
- The operations activity page is a filterable ledger: person (exact email), workspace or instance, kind, date range with a custom popover, time sort, rows per page up to 500, and per-row details with pivot links; filters apply on change inside a Turbo Frame, whose links all target `_top` so the frame serves only the on-change filter submits. `UI::TableComponent` (server-rendered table with caption, header, body and footer slots) ships with it and renders the operations workspaces list too. (#1164, #1166; modelrails_ui #201)
...
- A sortable table header now looks sortable before it is touched: `shared/_sortable_header` carries a muted up/down chevron at rest and the directional one on the sorted column, so the ledger's and the members table's sortable headers are told apart from plain ones without hovering. The glyphs are decorative; `aria-sort` on the `<th>` remains what assistive tech reads. New icon: `chevron_up_down`.
- The axe teardown's Turbo wait requires the page to stay quiet rather than believing the first quiet moment, so a frame swap that advances history no longer hands the audit a document mid-visit (an intermittent `aria-allowed-attr` on `<html>`).
- The operations activity ledger sorts by workspace name as well as by time (`ActivityLog.by_workspace_name`; instance-level rows last in either direction). Sorting by actor is a stated non-goal: names are encrypted and an under-the-cap in-memory sort would make the header conditional.
- The operations pages read as one system: `LifecycleHelper#lifecycle_status_badge` is the one status treatment (a soft badge named "Status: …", warning-tinted for a lock) on the workspaces list, a workspace's name line, a user's locked memberships and the ledger; `shared/_settings_page_header` takes `badges:` inline beside the title; `UI::TableComponent` is a filled card (`bg-surface`) and takes `scroll: :horizontal`, which scrolls only the table under a pinned toolbar and footer (the scroll region is named distinctly from the caption); `min-h-input` is the one spelling of the 44px floor in the area. A workspace's page keeps its lock control last under "Access" with a confirm that names the workspace, and links into the ledger; a user's page opens with every state as a badge (operator, suspended, sign-in blocked), the sentences after, and the controls in one row, with a confirm that names the person; the users search renders a hit as a list row with the address and a miss with a link into the ledger; the roster shows addresses and stacks at phone width. Ledger row details link to the person's page and to "only this workspace", the pivots say what they narrow, and every control that navigates the whole page (sort, rows, pager, range preset, pivot, Clear) returns focus to itself — or to the choice just made — via `data-focus-key` in `navigation_focus.js`. `UI::ListGroup` and the activity feed's list carry `role="list"`; `shared/_empty_state` is one hairline and its action link a 44px target. (modelrails_ui #201, #204)
...
- Vendored UI components carry a three-line doc pointer instead of header prose; the reference is the modelrails_ui gem's `docs/components/<name>.md` (0.16.0), guarded by a code-smell spec.
- Workspace identity is one control. The current workspace's logo, name and the viewer's role head the sidebar on every workspace page — solo users included — and opening it lists the other workspaces plus "All workspaces"; the "You're in …" context banner is gone. On phones the same control sits above the Overview / Projects / Settings tabs, showing the five most recently accessed workspaces with "All workspaces" as the overflow. The workspaces index keeps the control in an "All workspaces" state: on a phone it names the signed-in user, so the page opens with the same identity anchor desktop shows above the heading. (#1078, #1089, #1093)
- The five cleanup and retention sweeps (unattached blobs, expired sessions, WebAuthn challenges, activity-log retention, notification cleanup) run on the `low` queue, set in both `config/recurring.yml` and each job's `queue_as`, so an ad-hoc `perform_later` routes where the schedule does; the two `workspace_*` notifier sweeps stay on `default` (#894).
- `Workspace::NotAdmittableError`, `AlreadyMember`, and `AtCapacity` descend from a new `Workspace::AdmissionError`, so a fork's `rescue_from` has one class to name; `HomeWorkspaceProtectedError` is a lifecycle guard and deliberately not a subclass (#689).
...
- `shared/_sortable_header` takes a `frame:` local instead of hard-coding the members frame; the vendored combobox controller dispatches `change` on its hidden input after a selection (port to modelrails_ui pending).
...
- Joining a workspace is onboarding: `Workspace#admit`, the one membership-grant seam, stamps `onboarded_at`, so an invitee or link-joiner under the `none` preset never lands in the first-run wizard, and a refusal inside the wizard leaves it instead of looping. (#1145)
- An expired lockout resets the failed-attempt counter, so the next wrong password starts a fresh count instead of relocking at once. (#1140)
- Static rows on the workspaces, projects and resources indexes no longer highlight on hover (`text-interactive` on the sunken hover surface is 6.85:1, under AAA), and the resources index's title link meets the 44px target floor; that page gets its first axe audit. (#1141)
- Activity rows name who joined instead of "System": `ActivityLog#display_subject` falls back to the membership's member for `membership.created` rows written with no actor (a first signup's `after_create` runs before `Current.user` exists), and a workspace's own rows are attributed to the workspace via `Workspace#activity_workspace`. `invitation.updated` rows render again. (#1087)
- A spent magic link re-presented by its signed-in owner is answered truthfully — "you're already signed in" on a sign-in link, the registration confirmation on a signup link — instead of "expired". `GET` never consumes a token; the replay check is a predicate on the spent token and is fenced to the same address. (#1088)
- The workspace Profile page's Save button sat outside its own form because the logo section was nested inside it; the logo section is now a sibling of the name form. (#1079)
- The interpolation that carries a workspace's name is spelled `%{workspace_name}` everywhere (39 locale strings, the mailer templates, the join flow and their specs); `settings.en.yml` already used the long form, and the short one was a homonym of the domain noun that a vocabulary seam could silently fill. The name token is `%{workspace_name}` by convention and by the vocabulary gates (the seam above), not by a per-token spec. The i18n guide no longer claims the unused-keys check is ungated (#1108).
- The brand mark is a fork seam. `app/views/shared/_site_mark.html.erb` is a file that is one `<svg>` and nothing else, `merge=ours` like the brand words and colors, so a fork replaces it wholesale and never conflicts on it again; the favicon and PWA icon set in `public/` are `merge=ours` too. `_site_logo` is now just the lockup — mark and name as siblings, no wrapper — and the caller's link sizes the mark with `[&>svg]:h-*`; the `size`, `show_name`, and `color_class` locals are gone (no caller ever set the last, and only `name_class: "sr-only"` was ever varied). Two view specs assert the mark's contract and the lockup's shape; the extending guide's table, which had documented raw palette defaults since March, is rewritten, and the forking guide and `bin/fork`'s TODO list name the artwork (#1104).
- Every workflow job declares `timeout-minutes` (5–20, sized to each job's real duration), and the three `apt-get` steps carry a 5-minute step budget, so a runner-side hang fails in minutes instead of holding the required status for GitHub's 360-minute default; a template invariant keeps it that way (#1101).
- A notification whose delivery job never got enqueued is recovered instead of lost: noticed enqueues `Noticed::EventJob` after its own transaction commits, so a failed enqueue left committed rows and a burned idempotency key with no email or broadcast behind them. `noticed_events.dispatched_at` is stamped when the job starts, and `NotificationDispatchReconcileJob` (every 15 minutes, `default` queue) re-enqueues anything still unstamped between five minutes and 24 hours old, stamping each row as it goes so a backed-up queue cannot produce a duplicate fan-out. The migration backfills existing rows with their own `created_at` — a fork that skips it re-delivers its entire notification history on the first sweep (#927).
- `clear_solid_queue_finished_jobs` could never be claimed: the entry carried no `queue:` key, so Solid Queue routed it to `solid_queue_recurring`, which no worker in `config/queue.yml` polls. Every fork inherited the dead job — `config/recurring.yml` is not a `merge=ours` path (#894).
