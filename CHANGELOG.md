...
- `config/deploy.yml` passes Kamal's own validator — `stop_wait_time` (not a Kamal 2 key) is now `stop_timeout: 45`, and the `max-replicas: 1` docker option (not a `docker run` flag; Kamal 2 rolls over new-then-old regardless) is gone. Every `kamal` command died at config validation before this; the template-invariants spec now loads the file through `Kamal::Configuration` instead of asserting key names. The docs name the credentials key `.kamal/secrets` really reads (`config/credentials/production.key`).
