...
- Confirm dialogs open as `alertdialog`s with the message as their accessible description, so a screen reader announces title and message together on open — previously it announced the title and the Close button, and the message only on navigation. `shared/modal` accepts `role:`; the partial drops `size: :sm` (the role owns the width, `max-w-md`).
