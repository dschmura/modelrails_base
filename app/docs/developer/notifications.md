...
8. Dispatch from wherever the triggering event happens — `NotifierClass.with(record: ...).deliver(recipient)` when the call site knows the recipient, or `.deliver(nil)` when the notifier declares a `recipients` block. The two are not interchangeable; see *In-app gating lives in `recipients`* above for why, and `spec/code_smells/notifier_recipients_block_dispatch_spec.rb` for the fence
