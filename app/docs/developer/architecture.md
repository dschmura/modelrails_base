...
This does **not** mean `user.destroy` works. The audit trail was one of six columns pointing at `users` with no cleanup path, and the only one where both standard cleanups were wrong; the other five are open (#1248). The design also rests on one unstated database fact: `users.id` is `AUTOINCREMENT`, so ids are never reused. Under plain `rowid` SQLite hands the next insert `max(rowid)+1`, and deleting the highest-id user would silently re-attribute their audit rows to the next person who signs up.
