...
This is also the shape of the **creation verb** on an aggregate root — `Workspace#create_project`, `Workspace.create_owned`: a verb on the tenant root wrapping `lock!` + lifecycle guard + every row of the assembly (record plus its membership) in one transaction, returning the possibly-invalid record for form re-render, with deliberately no bang twin until a non-form caller exists. Any create that needs a second row to be usable takes this shape; two sequential writes reproduce the orphaned-record class #660/#676 closed. A follow-up that enqueues (an invitation's mail) sits outside the verb's transaction; `Workspace.create_for_owner_email` composes `create_owned` with `Invitation.bulk_invite!` in that order for exactly this reason.

Four kinds of sites lean on this property:

- **Capacity checks** — both tenant limits take the same shape. `Membership`'s and `Project`'s pre-flight validators produce the friendly error; an `after_create` invariant check inside the create transaction is the net that actually prevents over-capacity under concurrency. The two comparisons differ on purpose: `Workspace#at_capacity?` / `#at_project_capacity?` are `>=` (full once kept rows have *reached* `max_members` / `max_projects`, evaluated before the insert), while `Membership#enforce_capacity_invariant` and `Project#enforce_project_capacity_invariant` are a strict `>` because by the time they run the new row is already in the count — a `>=` there would roll back a legitimate at-the-limit insert. Do not route either net through `at_capacity?` / `at_project_capacity?`. A caller that wraps the insert in its own transaction must give the save a savepoint and use the bang form (`Workspace#create_project`): a non-bang save joins the outer transaction instead of opening a rollback boundary, and `ActiveRecord::Validations#save` rescues the `RecordInvalid` the net raises — the over-capacity row would then commit with the outer transaction.
...
- **Last-operator check** — `Operatorship#revoke_by_operator!` counts kept operatorships under the writer lock (`lock!` is the block's first statement, so it materializes `BEGIN IMMEDIATE`) before discarding one; a count of two or more followed by a single discard cannot reach zero, so it needs no after-write re-count. The Postgres caveat below applies in full — there is no parent row to lock.
...
**The trail has two write guarantees, and the difference is deliberate.** Workspace-domain rows (everything written through `Trackable`) are **best-effort**: they rescue and log rather than failing the business operation they describe, so that tier is an operational/product feature, not compliance-grade evidence. Account-security rows — the actions named in `ActivityLog::SECURITY_ACTIONS` — are **strict**: the audit row commits in the same transaction as the credential mutation it records, with no rescue, so a failed audit write fails the credential write. That is an evidence-over-availability trade, made on the reasoning that for a credential change an unrecorded mutation is worse than a refused one. See [Notifications § Security event audit coverage](/docs/developer/notifications) for the per-event table and what corroborates each row. Rows are read-only once persisted (#604), relation-level writes are fenced by `spec/code_smells/activity_log_immutability_spec.rb`, and every security-tier write is funnelled through `ActivityLog.record_security_event!` — fenced by `spec/code_smells/security_events_route_through_writer_spec.rb`. `actor:` and `visibility:` are parameters on that writer, defaulting to the self-event shape (the subject is the actor, personal visibility); a writer whose actor is someone else, like `Operatorship.grant!`/`#revoke!`, overrides both.

**An audit row outlives the people in it.** `actor_id` deliberately carries no foreign key: a user who has ever acted must still be deletable, and both obvious cleanups are wrong — `dependent: :destroy` deletes history to delete a person, and `dependent: :nullify` rewrites immutable rows to hide who acted (the immutability guard fences that spelling by name). Instead the row snapshots the actor's name into the encrypted `actor_name` column at write time, so the historical fact stops depending on the user row surviving — and stops following a later rename, which is what an audit table wants. `trackable_id` is polymorphic and has never had a foreign key; both columns now fail the same way rather than in opposite directions (#1122). Rows written before that column existed fall back to the association while it resolves and read as a former member once it does not — never as `System`, which would credit a person's action to a job. Those rows do still follow a rename, since they resolve live; that asymmetry is the price of not backfilling, and it ages out with them. There is no backfill on purpose: an `UPDATE` across the table to add a field is exactly the rewrite the immutability guarantee exists to prevent.

`user.destroy` works for a user who created nothing. The audit trail was one of six columns pointing at `users` with no cleanup path, and the only one where both standard cleanups were wrong; the other five now have a posture (#1248): a user's pending challenge rows go with them, and the creator of a project, resource or join link is refused with an error naming what they still own, because those columns are non-null and the choice between reassigning, nullifying with a fallback, and a name snapshot belongs to the fork that adds account deletion. `spec/code_smells/users_foreign_keys_have_a_cleanup_posture_spec.rb` derives every foreign key into `users` from the schema and fails on any without a posture. The design also rests on one unstated database fact: `users.id` is `AUTOINCREMENT`, so ids are never reused. Under plain `rowid` SQLite hands the next insert `max(rowid)+1`, and deleting the highest-id user would silently re-attribute their audit rows to the next person who signs up.
...
**Actors are parameters.** A model reads `Current` in one place: `Trackable`, where a callback cannot take an argument and a nil actor is a true answer (a job or the console has no user). Every domain verb takes its actor explicitly — `Workspace#admit(user, role:, granted_by:)`, `Membership#deactivate!(removed_by:)` — and a record's tenant is its own association, never `Current.workspace`, because the request's workspace and the record's can differ. The one best-effort writer outside the concern, `Membership::Ownership#record_ownership_demotion`, attributes the demotion to the owner stepping down (the membership's own user), which is exact from every caller and needs no ambient read (#1008). The `ModelRails/NoAmbientCurrentInModels` cop (`lib/rubocop`) fails any other `Current` read under `app/models` on commit; `Trackable`, `Tenanted`'s opt-in scope, and `Current` itself are its only exclusions. The rule and its reasoning: playbook `standards/backend/models.md` §What a model may read.
