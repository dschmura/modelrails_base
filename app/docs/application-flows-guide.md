---
title: Application Flows (builder's guide)
description: A readable builder's guide for developers and designers extending the template — clean wireframes paired with full-size prose explaining the why (the framework decision, seam, or guarantee) behind each screen, on a domain-model primer.
...
# Application Flows (builder's guide)

For **developers and designers extending this template**. Each flow shows the screens as a wireframe; the **"Why"** beneath it — rendered as normal, readable prose — explains the framework decision, the seam you'd extend, or the guarantee it gives you. The end-user flows are intentionally tight, so this page exists for *builders*, not users. Compare with the [low-fi map](/docs/application-flows), the [detailed copy reference](/docs/application-flows-detailed), and the [in-diagram annotated version](/docs/application-flows-annotated).
