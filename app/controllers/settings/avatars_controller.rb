    ...
    def update
        ...
        respond_to do |format|
          format.turbo_stream do
            render turbo_stream: turbo_stream.append("toast-cards",
              ...
              locals: { type: :error, message: t("settings.avatars.source_unavailable") }),
                   status: :forbidden
          end
          format.html { redirect_to edit_settings_profile_path, alert: t("settings.avatars.source_unavailable") }
        end
        return
      end
      ...
      if params[:avatar_source].present? && params[:avatar].blank?
        ...
        unless user.available_avatar_sources.include?(source)
          redirect_to edit_settings_profile_path, alert: t("settings.avatars.source_unavailable")
          return
        end
      ...
      if user.save
        ...
        respond_to do |format|
          ...
          format.html { redirect_to edit_settings_profile_path, alert: error_message }
        end
      end
    end
